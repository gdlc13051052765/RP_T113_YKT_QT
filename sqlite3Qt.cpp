...
char *collectionRecordDbFile = "/root/collectionRecord.db";//已采消费记录数据库
//黑名单数据库
...
configMsgSt Sqlite3QT::Sqlite_read_msg_from_configdb(void)
{
    ...
    pDevMsg.cardSector = atoi(azResult[hardNum+14]);//公共扇区号
    printf("公共扇区号 = %d \n",pDevMsg.cardSector);
...
* 创建时间： 2022-01-26 
==========================================================================================*/
void Sqlite3QT::Sqlite_update_ver(QString version)
{
    int err =-1;
    char * errmsg = NULL;
    sqlite3 *config_db =NULL;
    ...
    //修改设备ID
    QString str = "update config_db set version = '";
    str.append(version);
    str.append("'");
    QByteArray ba=str.toLatin1();
    char *sql=ba.data();
...
struct	dayConsumeRecordSt Sqlite3QT::sqlite3_consume_query_daymoney_db(QString daytime1, QString daytime2)
{
	...
	/* 打开数据库 */
	err = sqlite3_open_v2(recordDbFile, &recod_db, SQLITE_OPEN_READONLY, NULL);
    if( err ) {
	...
	{   
		...
		{	
			...
			// 取出第3列字段的值
			RecordStr.toalMoney += sqlite3_column_int(stmt, 3);
		}
...
/*=======================================================================================
* 函 数 名： sqlite3_move_collection_record_to_collection_db
* 参    数： 
* 功能描述:  把已经采集的交易记录搬运到已采消费记录数据库
* 返 回 值： 
* 备    注： 
* 作    者： lc
* 创建时间： 2022-01-26 
==========================================================================================*/
int Sqlite3QT::sqlite3_move_collection_record_to_collection_db()
{
    int err =-1;
    int recoreId;
    char * errmsg = NULL;
    sqlite3 *recod_db =NULL;
    sqlite3 *collectionRecord_db =NULL;
    sqlite3_stmt *stmt = NULL; // 用来取数据的
    ...
    /* 打开数据库 */
    err = sqlite3_open_v2(collectionRecordDbFile, &collectionRecord_db, SQLITE_OPEN_READWRITE, NULL);
    if( err ) {
        qDebug()<< "Can't open database: "<< sqlite3_errmsg(recod_db);
        sqlite3_close_v2(collectionRecord_db);
        //关闭数据库
        sqlite3_close_v2(recod_db);
        return 0xff;
    }
    ...
    //-1代表系统会自动计算SQL语句的长度
    QString str = "select *from consume where tag = 1";
    qDebug() << str;
	...
	{   
		...
		{	
            //把已采记录搬运到已采消费记录数据库
            recoreId = sqlite3_column_int(stmt, 1);
            QString str = "insert into consume values(";
            str.append(QString::number(sqlite3_column_int(stmt, 0)));
            str.append(",");
            str.append(QString::number(sqlite3_column_int(stmt, 1)));
            str.append(",");
            str.append(QString::number(sqlite3_column_int(stmt, 2)));
            str.append(",");
            str.append(QString::number(sqlite3_column_int(stmt, 3)));
            str.append(",'");
            str.append(QString::fromUtf8((char*)sqlite3_column_text(stmt, 4)));
            str.append("','");
            str.append(QString::fromUtf8((char*)sqlite3_column_text(stmt, 5)));
            str.append("','");
            str.append(QString::fromUtf8((char*)sqlite3_column_text(stmt, 6)));
            str.append("','");
            str.append(QString::fromUtf8((char*)sqlite3_column_text(stmt, 7)));
            str.append("')");
            qDebug() << "str";
            ...
            char *sql=ba.data();
            qDebug() << sql;
            if(sqlite3_exec(collectionRecord_db,sql,NULL,NULL,&zErrMsg) == SQLITE_OK)
            {
                //删除本条记录
                QString str = "delete from consume where id = ";
                str.append(QString::number(recoreId));
                str.append(" and tag = 1");
                str.append(";");
                QByteArray ba=str.toLatin1();
                char *sql=ba.data();
                qDebug() << sql;
...
/*=======================================================================================
* 函 数 名： sqlite3_query_consume_recorn_db
* 参    数： 
* 功能描述:  查找最后一条消费记录
* 返 回 值： 
* 备    注： 
...
* 创建时间： 2022-01-26 
==========================================================================================*/
int Sqlite3QT::sqlite3_query_consume_recorn_db(void)
{
	char tempdata[200];
...
* 创建时间： 2022-04-26 
==========================================================================================*/
int Sqlite3QT::sqlite3_blaknumber_query_db(int number)
{
	...
	int ret =-1;
	/* 打开数据库 */
    len = sqlite3_open_v2(blknumDbFile, &blknumber_db, SQLITE_OPEN_READONLY, NULL);
    if( len ) {
        qDebug()<< "Can't open database: "<< sqlite3_errmsg(blknumber_db);
        sqlite3_close_v2(blknumber_db);
        return 0xff;
    }
	if( len )
	{
	...
	if (sqlite3_prepare(blknumber_db, sql, -1, &stmt, NULL) == SQLITE_OK) {
		while (sqlite3_step(stmt) == SQLITE_ROW) {
			numberbak = sqlite3_column_int(stmt, 0);

            if(numberbak == number)//不区分大小写
            {
                qDebug() <<"黑名单卡";
                sqlite3_finalize(stmt);
                sqlite3_close(blknumber_db);
