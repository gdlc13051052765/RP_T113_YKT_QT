    ...
    ui(new Ui::setUp)
{
    ...
    //固件更新按键
    otaButton = new QPushButton("固件更新");
    otaButton->setFont(ft);//设置字体大小
    otaButton->setStyleSheet("QPushButton::focus {background-color: rgb(64, 224, 208); }");//焦点状态时控件颜色
    connect(otaButton, SIGNAL(clicked()), this, SLOT(otaButtonClicked()));
    ...
    setButtonLayout->addWidget(otaButton,1,1);//第1行，第1列开始
    // 设置水平间距
...
void setUp::keyReleaseEvent(QKeyEvent *ev)
{
    ...
    if (ev->key() == Qt::Key_F3)
    {
        ...
        system("/root/rebootQt.sh");
        //跳回主消费界面
    //     this->close();
    //    // mainConsumeInterface d ;
    //     mainConsumeInterface *aw = new mainConsumeInterface();
    //     aw->show();
